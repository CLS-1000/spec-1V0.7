...
class TopPacsWatch(WatchModule):
    entity_id = "watch_pacs"
    entity_name = "Top PACs"

    def __init__(self, names: list[str] | None = None) -> None:
        self._names = names or ["Example PAC", "America Future PAC"]
    ...
    def collect(self) -> WatchResult:
        signals = [
            Signal(
                ...
                weight=1.0,
                description=f"PAC watch hit: {name}",
                provenance=Provenance(source_uri="watch://pacs", source_name="PAC Watch", fetched_at=_now()),
            )

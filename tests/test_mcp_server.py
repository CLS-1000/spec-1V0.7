...
class TestFileVerdict:
    def test_files_verdict_and_persists(self, tmp_path):
    ...
    def test_missing_record_id_returns_error(self):
        result = tool_file_verdict({"outcome": "TP"})
        assert "error" in result

    def test_invalid_outcome_returns_error(self):
        result = tool_file_verdict({"record_id": "rec-1", "outcome": "MAYBE"})
        assert "error" in result

    def test_all_valid_outcomes_accepted(self, tmp_path):
        path = tmp_path / "verdicts.jsonl"
        with patch.dict(os.environ, {"SPEC1_VERDICTS_PATH": str(path)}):
            for outcome in ("TP", "FP", "TN", "FN", "PARTIAL"):
                result = tool_file_verdict({"record_id": "r1", "outcome": outcome})
                assert result["outcome"] == outcome


class TestGetVerdicts:
    def test_empty_store_returns_empty_list(self, tmp_path):
        with patch.dict(os.environ, {"SPEC1_VERDICTS_PATH": str(tmp_path / "v.jsonl")}):
            result = tool_get_verdicts({})
        assert result == []

    def test_returns_all_verdicts(self, tmp_path):
        path = tmp_path / "verdicts.jsonl"
        with patch.dict(os.environ, {"SPEC1_VERDICTS_PATH": str(path)}):
            tool_file_verdict({"record_id": "r1", "outcome": "TP"})
            tool_file_verdict({"record_id": "r2", "outcome": "FP"})
            result = tool_get_verdicts({})
        assert len(result) == 2
...
class TestGetCalibration:
    def test_returns_report_structure(self, tmp_path):
        intel_path = tmp_path / "intel.jsonl"
        verdicts_path = tmp_path / "verdicts.jsonl"
        intel_path.write_text(
            json.dumps({
                "record_id": "r1", "confidence": 0.8,
                "source_weight": 0.7, "analyst_weight": 0.6,
            }) + "\n"
        )
        env = {
        ...
        with patch.dict(os.environ, env):
            result = tool_get_calibration({})
        assert "report_id" in result
        assert "record_count" in result
        assert result["record_count"] == 1

    def test_include_proposals_flag(self, tmp_path):
        intel_path = tmp_path / "intel.jsonl"
        verdicts_path = tmp_path / "verdicts.jsonl"
        intel_path.write_text(
            json.dumps({
                "record_id": "r1", "confidence": 0.9,
                "source_weight": 0.9, "analyst_weight": 0.9,
            }) + "\n" * 5
        )
        env = {
            "SPEC1_STORE_PATH": str(intel_path),
        ...
        with patch.dict(os.environ, env):
            result = tool_get_calibration({"include_proposals": True})
